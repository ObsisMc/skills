...
4. Use every evidence array:
   ...
   - `created_issues` and `created_pull_requests`: global search cross-checks for work created on the requested date. They are authoritative for creation: an issue or PR that appears in `activities` but not here was acted on rather than created on that date.
...
6. Read attribution and timing carefully. A commit may be authored by a collaborator or coding agent but committed by the user. A merge or review remains user activity even when the user did not author the underlying commits. For `is_delayed_push: true`, write “今天推送/发布了此前完成的改动” rather than implying the implementation happened today. A push proves publication by the user, not authorship of every commit in the pushed range. For merged PRs, use `authored_by_user`, `merged_by_user`, and `merged_by_login`: say “我此前提交的 PR 今天被合入” when another person or automation merged it, and reserve “我合入了” for merges performed by the user. Apply the same care to issues, which carry the same ambiguity with none of the same field names. An `IssuesEvent` is not evidence that the user filed anything: read its `action` first, where `opened` is filing but `closed` and `reopened` are triage on an issue that usually predates the requested date and is often authored by someone else. Cross-check `created_issues`, which is authoritative for what the user actually created that day — an issue present in `activities` but absent there was not created on the requested date, regardless of how the event reads. Then check `issue_author_login` against the journal user before implying the user reported the problem, and reserve “我提了 issue” for `opened`; for a close, write “我关闭了……的 issue” and use `issue_state_reason` to distinguish a completed fix from one closed as not planned.
...
- Treat Issue filing, triage, closing, and investigation, PR creation and merging, code review, review feedback, and substantive comments or replies as first-class work. Closing or reopening an issue someone else filed is the user's work on that issue, not evidence that the user reported it.
