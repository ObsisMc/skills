...
4. Use every evidence array:
   - `commits`: global author and committer searches, commits expanded from same-day Push events, and commits read from the head branch of every pull request touched on the date, deduplicated by repository and SHA. A commit with `matched_via: push` may have been authored or committed earlier; use its `pushes`, original timestamps, and `is_delayed_push` fields to distinguish publication from implementation. A commit with `matched_via: pull_request` came from a pull request's head branch and carries a `pull_requests` back-reference with that pull request's number, state, and branch name; it is often not on the default branch yet. `authored_on_date` and `committed_on_date` separate a commit written on the date from one merely rebased, amended, or landed on it.
   ...
   - `updated_pull_requests`: PRs authored by the user and touched on the requested date, each with `head_ref_name`, `state`, `commits_on_date` read from the head branch, and `date_activity`. GitHub commit search indexes only a repository's default branch, so for work done on a feature branch this array is usually the only commit evidence. `date_activity` says whether the date carried commits, the PR's creation, its merge, or only discussion and metadata; a PR whose activity is only `discussion_or_metadata` was probably touched by someone else's comment and is not by itself a work theme.
5. Deduplicate overlapping evidence before writing. An opened PR can appear in `activities` and `created_pull_requests`, while its commits and merge can also appear separately. Treat them as one work theme. When a requested multi-day range includes both a commit's original date and its later push date, describe the implementation on the original date and mention the later push only if publishing, branch integration, or delivery was itself meaningful. Likewise, describe PR implementation and proposal on its creation/work date, then describe its later merge only as a delivery or project-state transition without repeating the implementation details. A branch commit and the squashed or rebased commit that lands on the default branch are one piece of work under two different SHAs, so SHA equality cannot detect this duplicate: match on the `pull_requests` back-reference instead, and write the implementation once, on the date the work was done.
6. Read attribution and timing carefully. A commit may be authored by a collaborator or coding agent but committed by the user. A merge or review remains user activity even when the user did not author the underlying commits. For `is_delayed_push: true`, write “今天推送/发布了此前完成的改动” rather than implying the implementation happened today. A push proves publication by the user, not authorship of every commit in the pushed range. Read `authored_on_date` together with `committed_on_date`: a commit authored earlier but committed on the requested date was rebased, amended, or cherry-picked rather than written that day, so write “今天整理/重新落盘了此前写的改动” instead of presenting it as new implementation. Rebasing one branch produces a whole run of such commits at once, and they are one act of maintenance rather than a day of authoring. For merged PRs, use `authored_by_user`, `merged_by_user`, and `merged_by_login`: say “我此前提交的 PR 今天被合入” when another person or automation merged it, and reserve “我合入了” for merges performed by the user.
7. Inspect `event_coverage` and `warnings`. GitHub exposes only a bounded recent user-event history. For dates outside that coverage, state that comments, replies, reviews, pushes, and other non-searchable activity may be incomplete. Never claim “no activity” when coverage is incomplete. Branch work has a further structural gap: commit search reaches only the default branch, and head-branch reading reaches only branches that already have a pull request, so a branch with neither a pull request nor a recoverable PushEvent leaves no commit evidence at all. Never treat an empty commit list as proof that no code was written.
...
3. For a relevant commit not adequately represented by a PR, inspect its changed files and patches. Use the repository the commit actually lives in, which for a fork-based pull request is its `head_repository` rather than the base repository:
...
- Distinguish four delivery states and never skip ahead: work sitting on a branch, work proposed in an open pull request, work merged into the default branch, and work released. A commit whose `pull_requests` entry is still `OPEN` has not been delivered, so say the work is still awaiting review rather than implying it landed.
...
- Treat a pull request advanced on the date, but neither created nor merged on it, as a real work theme when `commits_on_date` shows the user pushed to its branch. Describe what the new commits changed and that the pull request is still in flight.
